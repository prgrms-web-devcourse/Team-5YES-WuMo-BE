...
public class ReplyCommentServiceTest {
	...
	@Nested
	@DisplayName("registerReplyComment 를 사용해서 ")
	class registerReplyComment {

		ReplyCommentRegisterRequest replyCommentRegisterRequest =
				new ReplyCommentRegisterRequest(1L, "대댓글!!");

		@Test
		@DisplayName("후보지 댓글에 대댓글을 등록할 수 있다.")
		void success() {
			// Given
			...
			given(replyCommentRepository.save(any(ReplyComment.class))).willReturn(replyComment);
		...
		@Test
		@DisplayName("없는 댓글에 대댓글을 달 수는 없다.")
		void failedWithNotExistsComment() {
			// Given
			given(locationCommentRepository.findById(any(Long.class))).willReturn(Optional.empty());
		...
		@Test
		@DisplayName("모임의 외부인은 대댓글을 달 수 없다.")
		void failedWithOutMember() {
			// Given
			given(locationCommentRepository.findById(any(Long.class))).willReturn(Optional.of(locationComment));
